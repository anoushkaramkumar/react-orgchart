...
var ChartContainer = (0, _react.forwardRef)(function (_ref, ref) {
  ...
  var updateChartScale = function updateChartScale(newScale) {
    ...
    if (transform === "") {
      ...
      if (transform.indexOf("3d") === -1) {
        targetScale = Math.abs(windowVar.parseFloat(matrix[3]) * newScale);
        ...
        targetScale = Math.abs(windowVar.parseFloat(matrix[5]) * newScale);
  ...
  var exportPNG = function exportPNG(canvas, exportFilename) {
    ...
    var isFf = !!windowVar.sidebar;
    var isEdge = navigator.appName === "Microsoft Internet Explorer" || navigator.appName === "Netscape" && navigator.appVersion.indexOf("Edge") > -1;

    if (!isWebkit && !isFf || isEdge) {
      windowVar.navigator.msSaveBlob(canvas.msToBlob(), exportFilename + ".png");
    } else {
